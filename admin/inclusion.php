...
<div class="col-md-3">
<button type="button" id="new_sector_query" name="new_sector_query" class="btn btn-success" data-toggle="modal" data-target="#myModal" data-self_id="0" data-admin_id="<?= $admin_id;?>"  data-backdrop="static" data-keyboard="false">Add Inclusion</button>
</div> 
</div>
</div>
<div class="col-md-3">
      <ol class="breadcrumb">
        ...
        <li class="active">Inclusion List </li>		
      </ol>
  </div>
  </div>
  </div>
...
if(!empty($_GET["search"])){
	$result1=$con->query("select `id`,inclusion_title,inclusion_desc,inc_rate,admin_id from inclusion where admin_id='$admin_id'  && inclusion_title like '%".$_GET["search"]."%' order by id  desc");
}else{
	$result1=$con->query("select `id`,inclusion_title,inclusion_desc,inc_rate,admin_id from inclusion where admin_id='$admin_id' order by inclusion_title  desc");
}
...
$(document).ready(function(){
	...
	var s_url='ajaxpages/inclusions.php';
