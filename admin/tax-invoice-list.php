<?php include 'include/header.php'; ?>
 ...
 <div class="content-wrapper">
   ...
   $sql="SELECT `id`,`invoice_date`,`cust_name`,`sac`,`gst_no`,`pan_no`,`order_no`,`invoice_id`,`pax_no`,`total_amount`,`file_number`,`admin_id` FROM `tax_invoice` WHERE  cust_name like '%".$name."%' AND invoice_id like '%".$invoice_number."%'";

   $result1= mysqli_query($con,$sql);

	}else{

	$sql1="SELECT `id`,`invoice_date`,`cust_name`,`sac`,`gst_no`,`pan_no`,`order_no`,`invoice_id`,`pax_no`,`total_amount`,`file_number`,`admin_id` FROM tax_invoice  order by id  desc LIMIT 10";
	...
	<table class="table table-striped table-hover"><thead ><tr style="border:none;"><th colspan="6" style="border:none;"></th><th style="border:none;">
	...
	<th style="border:none;">GST No.</th>
	
	<th style="border:none;">PAN No</th>
	...
	<th colspan="4" style="border:none;text-align:center;">Action </th>
...

  {
	...
	echo'<td style="border:none;">' . $row1['gst_no'] .'</td>';
	
	echo'<td style="border:none;">' . $row1['pan_no'] .'</td>';
	...
	<button type="button" id="update_invoice" name="update_invoice"  class="btn btn-danger"  data-toggle="modal" data-target="#myModal" data-self_id="'.$row1['id'].'">Edit</button>'. "</td>";
	
    echo '<td style="border:none;">
	<button type="button" id="tax_invoice" name="tax_invoice"  class="btn btn-success"  data-toggle="modal" data-target="#myModal" data-self_id="'.$row1['id'].'" data-admin_id="'.$row1['admin_id'].'">View</button>'."</td>";
...
$(document).ready(function(){
  ...
  fetch_data(a_url1,a_request,a_response);
   ...
   insert_data(a_url1,a_request1,a_response1);
	...
	var a_url2='ajaxpages/update_invoice.php';

	$("body").on("click","#update_invoice",function(){ 
