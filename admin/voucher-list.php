<?php include 'include/header.php'; ?>
 ...
 <div class="content-wrapper">
	 ...
	 <div class="box box-primary"> 
			...
			<div class="col-md-3">
				...
				<div class="form-group">
				 ...
				 <input type="search" name="file_no" class="form-control" />
  ...
  $file_no = mysqli_real_escape_string($con,$_POST["file_no"]);

  $guest_name = mysqli_real_escape_string($con,$_POST["search_name"]);

  $phone_no = mysqli_real_escape_string($con,$_POST["search_mobile"]);
  

  $sql="SELECT `id`,`file_no`,`guest_name`,`contact_no`,`departure_time`,`status`,`admin_id` FROM `voucher_details` WHERE admin_id='".$admin_id."' &&  file_no LIKE '%".$file_no."%' And contact_no LIKE'%".$phone_no."%'And file_no LIKE'%".$file_no."%'";

  $result1= mysqli_query($con,$sql);

	}else{

	$sql1="SELECT `id`,`file_no`,`guest_name`,`hotel_name`,`contact_no`,`departure_time`,`status`,`admin_id` from `voucher_details`  where admin_id='".$admin_id."' order by id  desc";
