...
<div class="col-md-3">
<button type="button" id="new_sector_query" name="new_sector_query" class="btn btn-success" data-toggle="modal" data-target="#myModal" data-self_id="0" data-admin_id="<?= $admin_id;?>"  data-backdrop="static" data-keyboard="false">Add Vehicle</button>
</div> 
</div>
</div>
<div class="col-md-3">
...
if(!empty($_GET["search"])){
	$result1=$con->query("select `id`,name,admin_id from vehicle where admin_id='$admin_id'  && name like '%".$_GET["search"]."%' order by trim(name)  asc");
}else{
	$result1=$con->query("select `id`,name,admin_id from vehicle where admin_id='$admin_id' order by trim(name)  asc");
}
...
$(document).ready(function(){
	...
	var s_url='ajaxpages/vehicle.php';
