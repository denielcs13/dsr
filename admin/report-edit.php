                        ...
                        if (isset($_POST["submit_dsr"])) {
                            ...
                            if ($ins) {
                                ...
                                $no_rooms =
                                        $_POST["no_rooms"];
                                for ($i =
                                0; $i < count($sector_code); $i++) {
                                    if ($sector_code[$i] != '' && $sector_code_id[$i] != '') {
                                        $result1 =
                                                $con->query("select `id`,sec_code,sec_name,itinerary,itinerary_desc from sector_code where id='" . $sector_code_id[$i] . "'")->fetch_assoc();
                                        $insert_dsr =
 ...
 <input type="hidden" class="form-control" name="sector_code_id[]" id="sector_code_id" value="' . $data_list["sector_code_id"] . '"/><span id="auto_sector_code"></span>
                            ...
                            <table id="tr_clone" border="1" class="table table-hover table-bordered table-striped">
                                ...
                                <tbody>
                                    <?php
                                    $inclu_desc_detail =
                                            $con->query("select `id`,inclusion_id,inclusion_qty,inclusion_rate from online_dsr_inclusion where file_id2='" . $_GET["file"] . "' && query_id2 ='" . $_GET["query"] . "' &&  admin_id2='" . $_GET["admin"] . "' && dsr_status2='" . $_GET["load"] . "'");
                                    $inclu =
                                            $con->query("select `id`,inclusion_title,inclusion_desc,inc_rate,admin_id from inclusion where admin_id='" . $admin_id . "'");
                                    $option =
                                            '';
                                    ...
                                    $inclu->fetch_assoc()) {
                                        $option .=
                                                '<option value="' . $data_inc["id"] . '" data-rate="' . $data_inc["inc_rate"] . '">' . $data_inc["inclusion_title"] . '</option>';
                                    }
                                    $inclu1 =
                                            $con->query("select `id`,inclusion_title,inclusion_desc,inc_rate,admin_id from inclusion where admin_id='" . $admin_id . "'");
                                    while ($data_include =
                                    $inclu_desc_detail->fetch_assoc()) {
                                        ?>
                                        <tr><td colspan="2">
                                            ...
                                            <td colspan="2"><select id="inclusion_qty" name="inclusion_qty[]">
                                                    ...
                                                    ?>
                                                </select></td><td colspan="2"><input type="text" readonly value="<?= $data_include["inclusion_rate"]; ?>" name="inclusion_rate[]" id="inclusion_rate"></td></tr>	
                                        <?php
                                    }
                                    ?>
                                    <tr><td colspan="2">
                                            <select name="inclusion[]" id="inclusion"><option value='0'>Select Inclusion</option><?= $option; ?>
                                            </select>
                                        </td>
