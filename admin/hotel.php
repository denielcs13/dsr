...
<div class="col-md-3">
<button type="button" id="new_sector_query" name="new_sector_query" class="btn btn-success" data-toggle="modal" data-target="#myModal" data-self_id="0" data-admin_id="<?= $admin_id;?>"  data-backdrop="static" data-keyboard="false">Add Hotel</button>
</div> 
</div>
</div>
<div class="col-md-3">
...
if(!empty($_GET["search"])){
	$result1=$con->query("select `id`,hotel_name,admin_id from hotel where admin_id='$admin_id'  && hotel_name like '%".$_GET["search"]."%' order by trim(hotel_name)  asc");
}else{
	$result1=$con->query("select `id`,hotel_name,admin_id from hotel where admin_id='$admin_id'order by trim(hotel_name)  asc");
}
