            ...
            <div class="col-md-12"> 
                <div class="box box-primary table-responsive"> 
                    ...
                    if (isset($_POST["submit"])) {
                        $hotel_name =
                                mysqli_real_escape_string($con, $_POST["search_hotel"]);
                        $file_no =
                                mysqli_real_escape_string($con, $_POST["search_file"]);
                        $phone_no =
                                mysqli_real_escape_string($con, $_POST["search_mobile"]);
                        $account_no =
                                mysqli_real_escape_string($con, $_POST["search_account"]);

                        $sql =
                                "SELECT `id`,`hotel_name`,`file_no`,`phone_no`,`account_no`,`mail_id`,`status`,`admin_id`,`branch_name` FROM `vendor_payment` WHERE admin_id='" . $admin_id . "' && hotel_name like '%" . $hotel_name . "%' And file_no LIKE '%" . $file_no . "%' And phone_no LIKE'%" . $phone_no . "%'And account_no LIKE'%" . $account_no . "%'";
                        $result1 =
                                mysqli_query($con, $sql);
                    } else {
                        $sql1 =
                                "select `id`,`hotel_name`,`file_no`,`phone_no`,`account_no`,`mail_id`,`status`,`admin_id`,`branch_name`  from vendor_payment  where admin_id='" . $admin_id . "' order by id  desc";
                        $result1 =
...
<script>
    $(document).ready(function () {
        ...
        $("body").on("click", "#update_calling", function () {
            ...
            var update_from = new FormData($("#form-update-vendor")[0])
